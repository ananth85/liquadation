...
Now supports custom OpenAI-compatible providers and includes 150 realistic liquidation reasons
"""
...
class LLMGenerationAgent(BaseAgent):
    """Agent responsible for generating content using custom LLM provider or fallback methods"""
        ...
        if api_key and openai:
            try:
                ...
                if base_url:
                    ...
                    self.logger.info(f"Custom LLM provider initialized: {base_url}")
                else:
                    self.client = openai.OpenAI(api_key=api_key, timeout=timeout)
                    self.logger.info("Standard OpenAI client initialized")
            ...
            self.logger.warning("No LLM API key provided - will use fallback synthetic generation")

    def _test_connection(self):
        """Test the LLM connection without making a full request"""
        try:
            if self.client:
                # Simple test to verify the client is working
                self.logger.info(f"LLM client ready with model: {self.model}")
        except Exception as e:
    ...
    async def generate_from_prompt(self, prompt: str) -> Dict[str, Any]:
        """Generate company data and liquidation documents from a prompt"""
        self.logger.info(f"Generating from prompt: {prompt[:100]}...")
        
        try:
            if self.client:
                return await self._generate_with_custom_llm(prompt)
            else:
                return await self._generate_fallback(prompt)
        except Exception as e:
            self.logger.error(f"Generation failed, falling back to synthetic: {e}")
            return await self._generate_fallback(prompt)

    async def _generate_with_custom_llm(self, prompt: str) -> Dict[str, Any]:
        """Generate content using custom LLM provider"""
        if not self.client:
            raise RuntimeError("Custom LLM client not available")

        enhanced_prompt = f"""You are an expert liquidation document generator for Australian companies. 
Generate realistic company data and liquidation scenarios based on this prompt: {prompt}

Return a JSON response with the following structure:
{{
    "company_name": "Company Name Pty Ltd",
    "abn": "11 111 111 111",
    "acn": "111 111 111",
    "entity_type": "Australian Private Company",
    "industry": "Professional Services",
    "address": {{"street": "...", "suburb": "...", "state": "NSW", "postcode": "2000"}},
    "liquidation_reason": "Detailed explanation of why the company is being liquidated",
    "liquidation_type": "Creditors' Voluntary Liquidation",
    "directors": ["Director Name"],
    "assets_value": 50000,
    "liabilities_value": 120000,
    "major_creditors": ["Creditor 1", "Creditor 2"],
    "liquidator": {{"name": "John Smith", "registration": "12345", "firm": "Smith & Associates"}}
}}

Make it realistic and specific to Australian corporate law and liquidation procedures."""
        ...
        try:
            response = await asyncio.wait_for(
                asyncio.to_thread(
                    ...
                    model=self.model,
                    messages=[
                        {"role": "system", "content": "You are an expert in Australian corporate liquidation procedures."},
            ...
            self.logger.error(f"Custom LLM generation failed: {e}")
            raise
    ...
    async def _extract_data_from_text(self, text: str, original_prompt: str) -> Dict[str, Any]:
        ...
        # Fallback data extraction from text (same as before but noting custom LLM)
        data = await self._generate_fallback(original_prompt)
        
        # Update to note it came from LLM but had to be processed
        data["source"] = "custom_llm_extracted"
        data["original_llm_response"] = text[:500]  # First 500 chars for debugging
