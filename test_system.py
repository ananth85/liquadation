...
async def test_system():
    ...
    try:
        ...
        supervisor = SupervisorAgent(
            ...
            openai_api_key=None,  # Use fallback generation
        ...
        try:
            ...
            rag_task_id = await supervisor.rag_knowledge_agent.add_task("get_document_structure", {
                "query_type": "document_structure",
                "document_type": "liquidation_resolution"
            })
            rag_task = await supervisor.rag_knowledge_agent.execute_next_task()
            ...
            if rag_task and rag_task.result and rag_task.result.get('success'):
                ...
                print("❌ RAG knowledge retrieval failed")
        ...
        try:
            ...
            template_task_id = await supervisor.template_engine_agent.add_task("render_document", {
                "document_type": "resolution",
                "company_data": test_data
            })
            
            template_task = await supervisor.template_engine_agent.execute_next_task()
            ...
            if template_task and template_task.result and template_task.result.get('success'):
                ...
                if template_task and hasattr(template_task, 'error_message'):
                    print(f"   Error: {template_task.error_message}")
        ...
        print("\n🎲 Test 4: Synthetic Data Generation")
        try:
            llm_task_id = await supervisor.llm_generation_agent.add_task("generate_from_prompt", {
                "prompt": "Generate liquidation documents for a test company in Sydney"
            })
            
            llm_task = await supervisor.llm_generation_agent.execute_next_task()
            
            if llm_task and llm_task.result and llm_task.result.get('success'):
                company_data = llm_task.result.get('data', {})
                company_name = company_data.get('company_name', 'Unknown')
                print(f"✅ Generated data for: {company_name}")
                success_count += 1
            else:
...
Date: 2025-01-15
            ...
            pdf_task_id = await supervisor.pdf_generation_agent.add_task("generate_pdf", {
                "content": test_content,
                "filename": "test_document.pdf",
                "document_type": "test"
            })
            
            pdf_task = await supervisor.pdf_generation_agent.execute_next_task()
        ...
        try:
            ...
            prompt_result = await supervisor.llm_generation_agent.add_task("generate_from_prompt", {
                "prompt": "Generate liquidation documents for a small tech company in Melbourne facing financial difficulties"
            })
            prompt_task = await supervisor.llm_generation_agent.execute_next_task()
            ...
            if prompt_task and prompt_task.result and prompt_task.result.get('success'):
                ...
                rag_guidance_task = await supervisor.rag_knowledge_agent.add_task("get_guidance", {
                    "query_type": "legal_clauses",
                    "document_type": "resolution",
                    "context": company_data
                })
                rag_guidance_task_result = await supervisor.rag_knowledge_agent.execute_next_task()
                ...
                template_result = await supervisor.template_engine_agent.add_task("render_document", {
                    "document_type": "resolution",
                ...
                if template_task and template_task.result and template_task.result.get('success'):
                    ...
                    print("❌ RAG-enhanced template rendering failed")
            else:
...
async def test_dependencies():
    ...
    optional_packages = [
        ...
        ("weasyprint", "Advanced PDF generation"),
